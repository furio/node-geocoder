...
OpenStreetMapGeocoder.prototype._geocode = function(value, callback) {
    ...
    if (typeof value == 'string') {
      ...
      for (var k in value) {
        var keyParam = k;
        var valParam = value[keyParam];

        if (k === "address")
            keyParam = "street";

        params[keyParam] = valParam;
      }
    }
...
OpenStreetMapGeocoder.prototype._formatResult = function(result) {
    ...
    if (result.hasOwnProperty("class") && result.class !== null) {
        extra["class"] = result.class;
    }
    if (result.hasOwnProperty("type") && result.type !== null) {
        extra["type"] = result.type;
    }
    if (result.hasOwnProperty("place_id") && result.place_id !== null) {
        extra["place_id"] = result.place_id;
    }
